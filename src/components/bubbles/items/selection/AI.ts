...
const startChat = (holder: Holder, container: HTMLDivElement, prompt: string) => {
    if (holder.aiClient) {
        ...
        const options = holder.editor!.userOptions;
...
const createAiPanelElement = (holder: Holder) => {
...
<!--                     <div style="line-height: 20px;margin-left: 10px"> AIGC </div>-->
                     <div style="width: 18px;height: 18px;display: inline-block" >
                ...
                `,
    onInit: (editor, tippyInstance, parentEle) => {
        const holder = {editor, tippyInstance} as Holder
        holder.aiPanelInstance = tippy(parentEle.querySelector("#ai")!, {
            content: createAiPanelElement(holder),
            appendTo: editor!.view.dom.closest(".aie-container")!,
            placement: "bottom",
            trigger: 'click',
            interactive: true,
            arrow: false,
            getReferenceClientRect: (() => {
                const {state, view} = editor as InnerEditor
